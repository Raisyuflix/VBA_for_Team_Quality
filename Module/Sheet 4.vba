Sub UploadToSQLServer()
    ...
    ' Set worksheet
    Set ws = ThisWorkbook.Sheets("DBM_Risk_Management_Summary") ' Ganti dengan nama sheet Anda
    ...
    For i = 2 To lastRow ' Mulai dari baris kedua untuk menghindari header
        ...
        If Not isValid Then
            MsgBox "Data pada baris " & i & " tidak valid: Kolom 1-6 dan 11-12 tidak boleh kosong.", vbExclamation
            GoTo NextIteration
        End If

        sql = "INSERT INTO DBM_Risk_Management_Summary (Bulan, Tahun, Periode, Departemen, Channel, Total_sample, Low_Risk, Medium_Risk, High_Risk, Extreme_Risk, Insert_Date, Insert_By) VALUES ("

        ' Tambahkan nilai dengan memeriksa apakah sel kosong
        For j = 1 To 12 ' Ganti dengan jumlah kolom yang sesuai
            If IsEmpty(ws.Cells(i, j).Value) Then
                sql = sql & "NULL"
            Else
        ...
        ' Reset nilai sel yang telah diproses, kecuali kolom 3, 11
        For j = 1 To 12 ' Ganti dengan jumlah kolom yang sesuai
            If j <> 2 And j <> 3 And j <> 11 Then ' Jika kolom bukan kolom 3, 11

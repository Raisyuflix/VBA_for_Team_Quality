Sub UploadToSQLServer()
    ...
    ' Set worksheet
    Set ws = ThisWorkbook.Sheets("DBM_LnD_Test_Monthly") ' Ganti dengan nama sheet Anda
    ...
    For i = 2 To lastRow ' Mulai dari baris kedua untuk menghindari header
        ...
        If Not isValid Then
            MsgBox "Data pada baris " & i & " tidak valid: Kolom 1-7 tidak boleh kosong.", vbExclamation
            GoTo NextIteration
        End If

        sql = "INSERT INTO DBM_LnD_Test_Monthly (Bulan, Tahun, Periode, Departemen, Nilai, Insert_Date, Insert_By) VALUES ("

        ' Tambahkan nilai dengan memeriksa apakah sel kosong
        For j = 1 To 7 ' Ganti dengan jumlah kolom yang sesuai
            If IsEmpty(ws.Cells(i, j).Value) Then
                sql = sql & "NULL"
            Else
        ...
        ' Reset nilai sel yang telah diproses, kecuali kolom 2, 3, 7
        For j = 1 To 7 ' Ganti dengan jumlah kolom yang sesuai
            If j <> 2 And j <> 3 And j <> 6 Then ' Jika kolom bukan kolom 2, 3, 7

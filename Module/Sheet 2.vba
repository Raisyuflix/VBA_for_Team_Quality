Sub UploadToSQLServer()
    ...
    ' Set worksheet
    Set ws = ThisWorkbook.Sheets("DBM_SOP_IK_Monthly") ' Ganti dengan nama sheet Anda
    ...
    For i = 2 To lastRow ' Mulai dari baris kedua untuk menghindari header
        ...
        If Not isValid Then
            MsgBox "Data pada baris " & i & " tidak valid: Kolom 1-5 dan 13-14 tidak boleh kosong.", vbExclamation
            GoTo NextIteration
        End If

        sql = "INSERT INTO DBM_SOP_IK_Monthly (Bulan, Tahun, Periode, Departemen, Jenis_Dokumen, Jumlah_Dokumen, Proses_Pembuatan, Proses_Review_Draft, Proses_Revisi, Sudah_Revisi, Pengesahan_Direktur, Perubahan_ke_template_eQMS, Insert_Date, Insert_By) VALUES ("

        ' Tambahkan nilai dengan memeriksa apakah sel kosong
        For j = 1 To 14 ' Ganti dengan jumlah kolom yang sesuai
            If IsEmpty(ws.Cells(i, j).Value) Then
                sql = sql & "NULL"
            Else
        ...
        ' Reset nilai sel yang telah diproses, kecuali kolom 3, 13
        For j = 1 To 14 ' Ganti dengan jumlah kolom yang sesuai
            If j <> 2 And j <> 3 And j <> 13 Then ' Jika kolom bukan kolom 3, 13
